    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6 flex justify-between items-center">
            <div>
                ...
                <p class="mt-2 text-sm text-gray-600">Manage meeting requests and schedule new meetings</p>
            </div>
            <button onclick="document.getElementById('createMeetingModal').classList.remove('hidden')" 
        ...
        <div class="mb-6">
            <h2 class="text-xl font-semibold text-gray-900 mb-4">
                ⏳ Pending Requests
                <span class="ml-2 px-2 py-1 bg-yellow-100 text-yellow-800 text-sm rounded-full">{{ $pendingRequests->count() }}</span>
            </h2>
        ...
        <div class="mb-6">
                ...
                <div class="grid grid-cols-1 gap-4">
                    ...
                    <div class="bg-white border border-gray-200 rounded-lg p-6 hover:shadow-md transition">
                        <div class="flex justify-between items-start">
                            ...
                            <form action="{{ route('teacher.meetings.cancel', $meeting) }}" method="POST" onsubmit="return confirm('Cancel this meeting?')">
                                @csrf
                                @method('DELETE')
            ...
            @else
                <div class="bg-white border border-gray-200 rounded-lg p-12 text-center">
                    ...
                    <p class="text-sm text-gray-400 mt-1">Schedule a meeting with a student to get started</p>
                </div>
            @endif
        </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $meeting->student->name ?? 'N/A' }}</td>
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $meeting->scheduled_at->format('M d, Y • h:i A') }}</td>
                            <td class="px-6 py-4 text-sm text-gray-600">{{ $meeting->duration_minutes }} min</td>
                            <td class="px-6 py-4">
...
<div id="createMeetingModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-lg bg-white">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-2xl font-bold text-gray-900">Schedule a Meeting</h3>
            <button onclick="document.getElementById('createMeetingModal').classList.add('hidden')" 
        ...
        <form action="{{ route('teacher.meetings.create') }}" method="POST" class="space-y-4">
            @csrf

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-2">Select Student *</label>
                <select name="student_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
            ...
            <div>
                ...
                <textarea name="description" rows="3" 
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500"
                          placeholder="Meeting agenda or topics to discuss..."></textarea>
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div class="flex">
                    ...
                    <p class="text-sm text-blue-700">A Zoom meeting link will be automatically created when you schedule this meeting.</p>
                </div>
            </div>

            <div class="flex justify-end space-x-3 pt-4">
                <button type="button" 
                        onclick="document.getElementById('createMeetingModal').classList.add('hidden')"
...
<div id="rejectModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-full max-w-lg shadow-lg rounded-lg bg-white">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-xl font-bold text-gray-900">Reject Meeting Request</h3>
        ...
        <form id="rejectForm" method="POST" class="space-y-4">
            @csrf
            <p class="text-sm text-gray-600">Are you sure you want to reject this meeting: <strong id="meetingTitle"></strong>?</p>
...
<script>
function showRejectModal(meetingId, meetingTitle) {
    document.getElementById('rejectModal').classList.remove('hidden');
    document.getElementById('rejectForm').action = `/teacher/meetings/${meetingId}/reject`;
